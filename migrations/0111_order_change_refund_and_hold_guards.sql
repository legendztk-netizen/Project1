DROP TRIGGER order_change_reserved_fund_guard;
      ...
      AND (shipment.id IS NULL OR shipment.version!=affected.shipment_version
        ...
        OR EXISTS(SELECT 1 FROM order_quantity_holds hold
          JOIN order_shipment_allocations allocation
            ON allocation.order_id=hold.order_id AND allocation.line_id=hold.line_id
          WHERE allocation.shipment_id=affected.shipment_id
            AND hold.shipment_id IS NULL AND hold.active=1))
  ) THEN RAISE(ABORT,'Affected shipments changed or remain held') END;
END;
--> statement-breakpoint
UPDATE application_schema_state SET version=112,updated_at=CURRENT_TIMESTAMP WHERE singleton=1;
