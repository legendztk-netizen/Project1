...
此入口提供胶管 SKU 完整参数、原币金额、币种、包装长度、图片覆盖及目标状态。子体继承系列属性、图片和销售规则。读取已存在 SKU 或其草稿后可编辑；新建时编号不可重复。系列属性编辑由 Ticket 02 的“系列 → 更多 → 编辑”弹窗交付；共享销售规则固定在“产品数据维护 → 销售、包装和价格”，Ticket 02 将其接入系列直接发布命令。本票提供共用系列命令契约。
